...
static int popSize(struct parseState *st, u_int32_t *sz)
{
    if(st->stkpos == 0)
        return -1;
    size_t stkpos = --st->stkpos;
...
static int parseArgNum(struct slice *b, struct parseState *st, u_int32_t *x)
{
    if(getU32(b, x) == -1)
        return -1;
    if(verbose) printf("argNum %lx\n", (unsigned long)*x);
    return 0;
}

/* pass a buffer as an argument */
static int parseArgAlloc(struct slice *b, struct parseState *st, u_int32_t *x)
{
    void *p;
    u_int32_t sz;
    ...
    || pushSize(st, sz) == -1)
        return -1;
    memset(p, 0, sz);
    *x = (u_int32_t)(u_long)p;
    if(verbose) printf("argAlloc %lx - allocated %x bytes\n", (unsigned long)*x, sz);
    return 0;
}

/* pass a buffer as an argument */
static int parseArgBuf(struct slice *b, struct parseState *st, u_int32_t *x)
{
    ...
    if(pushSize(st, sz) == -1)
        return -1;
    *x = (u_int32_t)(u_long)sliceBuf(bslice);

    if(verbose) printf("argBuf %lx from %d bytes\n", (unsigned long)*x, sz);
    dumpContents(sliceBuf(bslice), sz);
    return 0;
}

/* pass a buffer length as an argument */
static int parseArgBuflen(struct slice *b, struct parseState *st, u_int32_t *x)
{
    if(popSize(st, x) == -1)
        return -1;
    if(verbose) printf("argBuflen %lx\n", (unsigned long)*x);
    return 0;
}

/* make a file with buffer contents and set arg to fd open to the start of that file */
static int parseArgFile(struct slice *b, struct parseState *st, u_int32_t *x)
{
    static int num = 0;
    char namebuf[128];
    int fd;
    ...
    *x = fd;
    if(verbose) printf("argFile %lx - %ld bytes from %s\n", (unsigned long)*x, (u_long)sliceSize(bslice), namebuf);
    dumpContents(sliceBuf(bslice), sliceSize(bslice));
    return 0;
}

static int parseArgStdFile(struct slice *b, struct parseState *st, u_int32_t *x)
{
    int fd;
    unsigned short typ;
    ...
    if(fd == -1)
        return -1;
    *x = fd;
    if(verbose) printf("argStdFile %lx - type %d\n", (unsigned long)*x, typ);
    return 0;
}

static int parseArgVec64(struct slice *b, struct parseState *st, u_int32_t *x)
{
    u_int32_t *vec;
    int i;
    u_int8_t sz;
    ...
    if(sz && !vec)
        return -1;
    if(verbose) printf("argVec32 %lx - size %d\n", (unsigned long)(u_long)vec, sz);
    for(i = 0; i < sz; i++) {
    ...
    if(pushSize(st, sz) == -1)
        return -1;
    *x = (u_int32_t)(u_long)vec;
    return 0;
}

/* make a file with buffer contents and set arg to point to its filename */
static int parseArgFilename(struct slice *b, struct parseState *st, u_int32_t *x)
{
    static int num = 0;
    char namebuf[128];
    int fd;
    ...
    *x = (u_int32_t)(u_long)strdup(namebuf); /* note: we ignore memory leaks - exit/doneWork are perfect GCs */
    if(verbose) printf("argFilename %lx - %ld bytes from %s\n", (unsigned long)*x, (u_long)sliceSize(bslice), namebuf);
    dumpContents(sliceBuf(bslice), sliceSize(bslice));
    return 0;
}

static int
mkChild(u_int32_t *retPid)
{
    pid_t pid;
    int i;
...
static int parseArgPid(struct slice *b, struct parseState *st, u_int32_t *x)
{
    unsigned char typ;
    ...
    if(verbose) printf("argPid %lx - %d\n", (unsigned long)*x, typ);
    return 0;
}

/* Reference a previously defined argument as a new arg */
static int parseArgRef(struct slice *b, struct parseState *st, u_int32_t *x)
{
    ...
    *x = st->calls[ncall].args[narg];
    if(verbose) printf("argRef %lx - %d %d\n", (unsigned long)*x, ncall, narg);
    return 0;
}

static int parseArgVec32(struct slice *b, struct parseState *st, u_int32_t *x)
{
    u_int32_t elem;
    u_int32_t *vec;
    int i;
